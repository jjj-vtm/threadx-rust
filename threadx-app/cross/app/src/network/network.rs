use core::{ffi::c_void, mem::MaybeUninit, net::Ipv4Addr, ptr};
...
pub struct ThreadxTcpNetwork {
    sockets: Vec<NetxTcpSocket, 8>,
}
...
impl From<NxError> for stm32f4xx_hal::nb::Error<NetxTcpError> {
    fn from(value: NxError) -> Self {
        embedded_nal::nb::Error::Other(value.into())
    }
}

impl ThreadxTcpNetwork {
    pub fn initialize(ssid_str: &str, pw: &str) -> Result<ThreadxTcpNetwork, NxError> {
        let mut network = ThreadxTcpNetwork {
            sockets: Vec::new(),
        };
        ...
        let _ = network.sockets.push(NetxTcpSocket {
            socket_ptr: socket_ptr.as_mut_ptr(),
        });

        Ok(network)
    }
}
impl TcpClientStack for ThreadxTcpNetwork {
    type TcpSocket = NetxTcpSocket;

    type Error = NetxTcpError;

    fn socket(&mut self) -> Result<Self::TcpSocket, Self::Error> {
        Ok(self.sockets.pop().unwrap())
    }
    ...
    fn connect(
        ...
        match remote {
            core::net::SocketAddr::V4(socket_addr_v4) => {
                ...
                nx_checked_call!(_nx_tcp_client_socket_connect(
                    socket.socket_ptr,
    ...
    fn receive(
        ...
        nx_checked_call!(_nx_tcp_socket_receive(
            socket.socket_ptr,
            packet_ptr_ptr,
            NX_WAIT_FOREVER
        ))?;
        ...
        nx_checked_call!(_nx_packet_data_retrieve(
            packet_ptr,
    ...
    fn close(&mut self, socket: Self::TcpSocket) -> Result<(), Self::Error> {
        nx_checked_call!(_nx_tcp_socket_delete(socket.socket_ptr))?;
        Ok(())
    }
}
